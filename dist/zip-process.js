...
function processFile(file, options, callbacks) {
	...
	fs.readFile(file, function (err, data) {
		...
		if (err) {
			if (options && options.handleError) return options.handleError();else if (defaultOptions.handleError) return defaultOptions.handleError();else throw err;
		}

		processContent(data, options, callbacks).then(function (data) {

			var changed = !!data;

			if (changed) {
				// looking for output file name
				var fileName = options && (options.output || (options.replace ? options.fileName : undefined)) || defaultOptions.output;

				if (!fileName) throw 'No output file specified';

				fs.writeFileSync(fileName, data);
			}
		}).catch(function (e) {
			throw e;
		}); // enf of processContent
	}); // end of fs.readFile
}
